...
function SandPackFileWatcher({onLiveFilesChange}){
    ...
    const {activeProject, updateProjectsFiles} = useAppContext();
    ...
    useEffect(() => {
        ...
        if(hasChanged){
            updateProjectsFiles(updatedFiles);
        }
    }, [files]);
...
const PreviewPanel = ({project, activeFile, showCode}) => {
  ...
  return (
    <div className='h-full w-full'>
        <SandpackProvider key={project._id}
        template='react'
        files={sandpackFiles}
        customSetup={{dependencies}}
        options={{
            externalResources: [
                'https://csn.tailwindcss.com',
        ...
        >
            ...
            >
                {showCode && (
                    <SandpackCodeEditor showTabs showLineNumbers showInlineErrors wrapContent style={{
                        height: '100%',
                        flex: 1,
                        minWidth: 0
                    }}/>
